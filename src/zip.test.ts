...
test('ZIP に入れたファイルを、名前と中身そのまま取り出せる', async () => {
  const json = new TextEncoder().encode(JSON.stringify({ version: 1, records: [{ id: 'a' }] }))
  const photo = new Uint8Array(500).map((_, i) => i % 256)
  const zip = await createZip([
    { name: 'headache-data.json', data: json },
    { name: 'photo-abc123.jpg', data: photo },
  ])
  const entries = await readZip(zip)
  assert.equal(entries.length, 2)
  const back = new Map(entries.map((e) => [e.name, e.data]))
  assert.deepEqual(back.get('headache-data.json'), json)
  assert.deepEqual(back.get('photo-abc123.jpg'), photo)
})
